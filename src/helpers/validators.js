...
const filterColor = (color, object) =>
  ramda.filter(ramda.equals(color), object);
...
const isLengthValid = (color, num) => pipe(colorLength(color), lte(num));
// 1. Красная звезда, зеленый квадрат, все остальные белые.
export const validateFieldN1 = (shapes) => {
  const allPass = ramda.allPass([
    isRed("star"),
    isGreen("square"),
    isWhite("triangle"),
    isWhite("circle"),
...
export const validateFieldN5 = (shapes) => {
  ...
  const anyPass = ramda.anyPass(
    map((color) => isLengthValid(color, 3), colors)
  );
